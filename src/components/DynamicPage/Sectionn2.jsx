  ...
  <div className='flex-wrap overflow-x-hidden'>
    ...
    >
      <div className="lg:grid grid-cols-2 flex-wrap gap-8 p-10 mt-10 ">
        ...
        <img
          src={`http://localhost:8000/uploads/${location.AboutImage1.replace('\\', '/')}`}
          className="md:h-72 md:w-72 w-32 h-32 object-cover border-4 border-white shadow-lg rounded-lg p-2"
          alt="Protest Scene"
        />
       
       
        <img src={`http://localhost:8000/uploads/${location.AboutImage2.replace('\\', '/')}`} className="md:h-60 md:w-60 w-32 h-32 object-cover mt-20 border-4 border-white shadow-lg rounded-lg p-2"
          alt="Protest Scene" />
